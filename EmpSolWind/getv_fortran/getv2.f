...
c-----------------------------------------------------------------------
c
c ****** Find the solar wind speed given the expansion factor
c ****** and the coronal hole boundary distance.
c
c ****** This program can be used for the Wang-Sheeley-Arge (WSA)
c ****** model, as well as the PSI ad hoc coronal hole boundary
c ****** distance model.
c
c-----------------------------------------------------------------------
c
c ****** This version implements Nick Arge's formula from circa
c ****** August 16, 2006, when the WSA model is selected.
c
c-----------------------------------------------------------------------
c
c ****** Updates and bug fixes:
c
c        06/01/2010, ZM, Version 1.00:
c
c         - Original version of the program.
c         - This program was derived from GETV_WSA version 1.00
c           of 03/13/2008.
c         - This program can be used for both the WSA model and the
c           PSI ad hoc coronal hole boundary distance model.
c
c        08/25/2010, JL, Version 1.01:
c
c         - The program now computes the density and temperature
c           using the ad hoc prescription from mas47f_ip.F
c           and outputs files with these quantities.
c           The data is output in real units (#/cm^3 and degrees K).
c
c        04/29/2021, RC, Version 1.10:
c
c         - Added new updated WSA model from [McGregoretal 2011].
c           To use, set "-model WSA2" on the command line.
c           This model does not floor the velocity,
c           and has a new parameter called "WSA_C5" (default 3.0).
c
c-----------------------------------------------------------------------
c
c#######################################################################
